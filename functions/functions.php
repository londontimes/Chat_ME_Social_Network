...
function get_posts(){
	...
	$own = $user_id;
	$friend = array();
	$get_friend_id = "select f_id from friends where user_id='$user_id'";
	$friend_id = mysqli_query($con, $get_friend_id);
	while($rowf = mysqli_fetch_assoc($friend_id)){
	$friend[] = $rowf;
	}
	$per_page = 4;
	...
	while($row_posts = mysqli_fetch_array($run_posts)){
		...
		foreach($friend as $k){
			if($k['f_id'] == $user_id || $own == $user_id){
				if($own == $user_id){
					if(count($own_post_id) == 0){
						$own_post_id[] =$post_id;
		...
		if($content=="No" && strlen($upload_image) >= 1){
			...
			";
					...
					for ($i=0;$i<count($own_post_id);$i++ ) {
						if($own_post_id[$i] != $post_id){
							$own_post_id[$i]= $post_id;
